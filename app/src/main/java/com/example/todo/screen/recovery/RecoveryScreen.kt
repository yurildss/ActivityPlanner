...
import com.example.todo.screen.sign_up.SignUpScreenViewModel
...
fun RecoveryPasswordScreen(
    ...
    viewModel: RecoveryPasswordScreenViewModel = hiltViewModel(),
    onEmailSend: () -> Unit
){
    ...
    Scaffold(
        modifier = modifier
            .fillMaxSize()
            .testTag("recovery_Password_Screen"),
        ...
        )
        {
            Column(
                ...
                Button(
                    onClick = { viewModel.sendRecoveryEmail(onEmailSend) },
                    modifier = Modifier
                        .padding(10.dp)
                        .size(350.dp, 50.dp),
                    colors = ButtonDefaults.buttonColors(Color(0xFFB2F02C))
                ){
                    Text("Send", color = Color.Black, fontSize = 20.sp)
